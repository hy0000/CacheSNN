...
class MemAccessBusToPipeLineMemoryBusTest extends AnyFunSuite {
  ...
  val complied = simConfig.compile(new MemAccessBusToPipeLineMemoryBus)
  ...
  test("MemAccessBus to PipeLineMemoryBus test"){
    complied.doSim(1022600498){dut =>
      dut.clockDomain.forkStimulus(2)
      SimTimeout(10000)
      val mem = PipelineMemorySim(dut.io.output, dut.clockDomain, 2)
